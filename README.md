...
Final reviewer-facing deliverables:
...
- `Summary`: total trade count, counterparties, netting sets, AddOn, PFE, RC, EAD, and AddOn by asset class chart
- `Checks`: pass/fail controls comparing Excel formulas with Python outputs
- `Trade_Calc`: trade-level maturity factor, supervisory duration, delta, adjusted notional, and effective notional formulas
- `AddOn_Calc`, `Asset_Class_AddOn`, `Netting_Set_Calc`, `Counterparty_Calc`: formula-based aggregation and exposure reconciliation
- `Python_*` sheets: imported Python outputs used as reconciliation targets
